...
pub fn generate_schemas<T: schemars::JsonSchema>() -> Result<(), Error> {
    ...
    write_schema_file(schema, &schema_name)?;

    let pcf_schema = gen_pcf_with_extension::<T>()?;

    write_schema_file(pcf_schema, &format!("pcf-{schema_name}"))?;

    Ok(())
}

pub fn gen_pcf_with_extension<T: schemars::JsonSchema>() -> Result<RootSchema, Error> {
    let type_name = get_type_name::<T>()?;
